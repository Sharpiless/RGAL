...
def main_worker(gpu, ngpus_per_node, args):
    ...
    # Logger
    ############################################
    log_name = 'R%d-%s-%s' % (args.rank, args.dataset,
                              args.model) if args.multiprocessing_distributed else '%s-%s' % (args.dataset, args.model)
    args.logger = datafree.utils.logger.get_logger(
        log_name, output='checkpoints/scratch/log-%s-%s.txt' % (args.dataset, args.model))
    if args.rank <= 0:
        # print args
    ...
    # Setup dataset
    ############################################
    num_classes, train_dataset, val_dataset = registry.get_dataset(
    ...
    def warm_up_with_multistep_lr(epoch): return (
        epoch+1) / args.warm_up_epoches if epoch < args.warm_up_epoches else 0.1**len([m for m in milestones if m <= epoch])
    ...
    if args.evaluate_only:
        ...
        print('[Eval] Acc@1={acc1:.4f} Acc@5={acc5:.4f} Loss={loss:.4f}'.format(
            acc1=acc1, acc5=acc5, loss=val_loss))
        return
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        _best_ckpt = 'checkpoints/scratch/%s_%s.pth' % (
...
def train(train_loader, model, criterion, optimizer, args):
    global best_acc1
    loss_metric = datafree.metrics.RunningLoss(
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        if args.print_freq > 0 and i % args.print_freq == 0:
            (train_acc1, train_acc5), train_loss = acc_metric.get_results(
            ), loss_metric.get_results()
